...
//int map[MAX][MAX]; /*0表示空白，1表示战机*的区域，2表示敌人战机的位置。 3表示上下围墙，4表示左右围墙,5表示子弹的位置*/ 
//int score;
...
//i = high; 
//for (j = 1; j <= width; j++) 
//map[i][j] = 3; map[bullet_x][bullet_y] = 5;
///*这里是战机大小的初始化开始*/ 
...
//map[position_x + 1][position_y + 1] = 1; /*** 初始化结束 **/
///* 敌人战机的初始化 */
...
//SetConsoleCursorInfo(GetStdHandle(STD_OUTPUT_HANDLE), &cursor_info); } void gotoxy(int x, int y)//清理一部分屏幕
...
//enemy_x = 0;
//enemy_y = rand() % width;
//bullet_x = 0; } if (enemy_x > high)//当飞机超出区域 
//{ enemy_x = 0; 
//enemy_y = rand() % width; }
//if (speed == 1) 
...
//if (input == ' ') { bullet_x = position_x - 1; bullet_y = position_y; } 
//} 
//} 
//void show()//展示的内容 
//{ gotoxy(0, 0);
